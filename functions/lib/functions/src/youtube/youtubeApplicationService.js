...
exports.YoutubeApplicationService = void 0;
const firestoreYouTubeRepository_1 = require("./firestoreYouTubeRepository");
const realYouTubeClient_1 = require("./realYouTubeClient");
class YoutubeApplicationService {
    repository;
    client;
    constructor(repository, client) {
        this.repository = repository || new firestoreYouTubeRepository_1.FirestoreYouTubeRepository();
        this.client = client || new realYouTubeClient_1.RealYouTubeClient();
    }
    // ---------------------------------------------------------------------------
     ...
     */
    async fetchChannelUploads(input) {
        ...
        const playlistPage = await this.client.getPlaylistItems(uploadsPlaylistId, input?.pageToken, input?.maxResults || 25);
        ...
        for (const fetchedVideo of fetchedVideos) {
            ...
            if (existing) {
                const sourceHasChanged = existing.sourceSnapshot.sourceTitle !== snapshot.sourceTitle ||
                ...
                const newCandidate = {
                    ...
                    editorialDraft: {
                        ...
                        titleEn: '',
                        excerptAr: snapshot.sourceDescription.substring(0, 150) +
    ...
    async saveCandidateFromSource(snapshot, externalVideoId) {
        ...
        let nextVersion = 1;
        let currentLifecycle = 'PendingReview';
        let currentEditorialDraft;
        if (existing) {
    ...
    async updateEditorialDraft(id, updates) {
    ...
    async rejectCandidate(id, reviewedVersion) {
        const candidate = await this.repository.getCandidate(id);
        if (!candidate)
            throw new Error('Candidate not found');
        if (candidate.status !== 'PendingReview') {
    ...
    async acceptCandidate(id, reviewedVersion, mediaRegisterFn) {
        const candidate = await this.repository.getCandidate(id);
        if (!candidate)
            throw new Error('Candidate not found');
        if (candidate.status !== 'PendingReview') {
...
exports.YoutubeApplicationService = YoutubeApplicationService;
//# sourceMappingURL=youtubeApplicationService.js.map