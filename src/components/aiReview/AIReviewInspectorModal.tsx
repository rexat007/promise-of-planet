...
export const AIReviewInspectorModal: React.FC<AIReviewInspectorModalProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div className="p-4 sm:p-6 border-b border-gray-100 dark:border-gray-800 flex items-start justify-between gap-4 bg-gray-50/50 dark:bg-gray-800/30">
          <div className="space-y-1 min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-300 border border-emerald-200/60 dark:border-emerald-800">
        ...
        <div className="px-4 sm:px-6 py-2.5 bg-emerald-50/70 dark:bg-emerald-950/30 border-b border-emerald-100 dark:border-emerald-900/40 flex items-center gap-2 text-xs text-emerald-800 dark:text-emerald-300">
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-6">
          {/* Metadata Grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 p-4 bg-gray-50 dark:bg-gray-800/40 rounded-xl border border-gray-100 dark:border-gray-800 text-xs">
            <div>
              <span className="text-gray-500 dark:text-gray-400 block">{isAr ? 'حالة التنفيذ' : 'Execution State'}</span>
              <span className="font-bold text-gray-800 dark:text-gray-200">{getExecutionStateLabel(review.executionState, isAr)}</span>
            </div>
            <div>
          ...
          {isStale && (
            <div className="p-4 rounded-xl bg-amber-50 dark:bg-amber-950/50 border border-amber-200 dark:border-amber-800/60 flex items-start gap-3 text-xs text-amber-900 dark:text-amber-200">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-bold text-gray-900 dark:text-white flex items-center gap-2">
                ...
                <span>{isAr ? 'الملاحظات والنتائج الاستشارية المرصودة' : 'Structured Advisory Findings'}</span>
                <span className="px-2 py-0.5 rounded-full text-xs bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 font-bold">
                ...
                {review.findings.map((finding: AIReviewFinding, index: number) => {
                  ...
                  return (
                    <div 
                      ...
                      className="p-4 rounded-xl bg-white dark:bg-gray-800/60 border border-gray-200 dark:border-gray-700/80 space-y-3"
                    >
                      {/* Finding Tags */}
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div className="flex flex-wrap items-center gap-2">
                          <span className="px-2.5 py-0.5 rounded-md text-xs font-bold bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200">
                            {getCategoryLabel(finding.category, isAr)}
                          </span>
                          {getSeverityBadge(finding.severity)}
                          {finding.flag && (
                            <span className="px-2.5 py-0.5 rounded-md text-xs font-semibold bg-purple-50 text-purple-700 dark:bg-purple-950/60 dark:text-purple-300 border border-purple-200 dark:border-purple-800">
                        ...
                        {finding.affectedField && (
                          <span className="text-[11px] font-mono text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-900 px-2 py-0.5 rounded-md border border-gray-100 dark:border-gray-800">
                      ...
                      {/* Advisory Suggestion */}
                      {displaySuggestion && (
                        <div className="p-3 rounded-lg bg-emerald-50/60 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-900/40 text-xs space-y-1">
                          <div className="flex items-center gap-1.5 font-bold text-emerald-800 dark:text-emerald-300">
                            <Lightbulb className="w-3.5 h-3.5 text-emerald-600" />
                            <span>{isAr ? 'توصية تحريرية استشارية:' : 'Advisory Editorial Suggestion:'}</span>
                          </div>
                          <p className="text-gray-700 dark:text-gray-300 leading-normal pl-5 rtl:pl-0 rtl:pr-5">
                            {displaySuggestion}
                          </p>
                        </div>
                      )}
        ...
        {/* Footer */}
        <div className="p-4 sm:p-5 border-t border-gray-100 dark:border-gray-800 bg-gray-50/60 dark:bg-gray-800/40 flex flex-wrap items-center justify-between gap-3">
          ...
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-xl bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-800 dark:text-white font-bold text-xs transition-colors cursor-pointer"
          >
